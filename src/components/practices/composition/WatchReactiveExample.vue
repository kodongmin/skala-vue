...
// 1) 변수명 그대로 감시 -> reactive 객체는 자동으로 deep 감시가 걸림 (이전/현재 값이 똑같이 찍히는 한계 있음)
watch(state, (newVal, oldVal) => {
  logAutoDeep.value = `[자동 deep] 가격 변동! 이전가격인척하는:${oldVal.price}원 ➡️ 현재가격:${newVal.price}원`
})

// 2) 화살표 함수로 특정 속성만 감시 -> 진짜 이전 값 추적 가능
watch(
  () => state.price,
  (newPrice, oldPrice) => {
...
<template>
  <div class="practice-section">
    ...
    <div class="monitor">
      <p>👁️ 1) state 변수 통째로 감시 (deep 자동화)</p>
      <p>{{ logAutoDeep }}</p>
    ...
    <div class="monitor target">
      ...
      <p>{{ logTarget }}</p>
      <small>※ 성공: 과거의 원본 가격이 정확히 보존된다.</small>
    </div>
  </div>
</template>
