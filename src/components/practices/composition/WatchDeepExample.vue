...
const user = ref({ name: '고동민', age: 20 })
const logDeep = ref('아직 반응 없음')
const logTarget = ref('아직 반응 없음')

// 해결책 1: deep 옵션을 켜서 객체 하위 속성 전체 감시하기 (이전 값과 현재 값이 동일하게 나옴에 유의)
watch(
  user,
  (newVal) => {
...
<template>
  <div class="practice-section">
    ...
    <div class="monitor">
      <p>👁️ 1) deep: true 모니터 (전체 감시)</p>
      <p>{{ logDeep }}</p>
    </div>
    <div class="monitor target">
      <p>🎯 2) 화살표 함수 모니터 (나이만 타겟 감시, 과거 값 추적 가능)</p>
      <p>{{ logTarget }}</p>
    </div>
  </div>
</template>
